(function(global)
{
	...
	commonController.boardManagement.memberInvitation =
	(function()
	{

		//회원 초대
		var urlMemberInvitation = commonController.defaultPath + "/letter/createLetter";
		...
		function _init()
		{
			...
			function(e)
			{
				...
				console.log($(selectedRow).children(":first-child").text()); // 선택된 행의 첫번째 값!!

				commonController.confirmDialog('모임 초대', '이 회원을 모임으로 초대하시겠습니까?',
				function()
				{
					var selectedMember =
					{
						meeId : $.cookie('meeid') + "",
						letTomem : $(selectedRow).children(":first-child").text() + "",
						letFrommemname : $.cookie("name") + ""
					};

					console.log('[모임 초대시에]');
					console.log(selectedMember);

					commonController.ajax.accessData(urlMemberInvitation, selectedMember,// 주소값, 파라미터, 성공실패
					function(json) // 성공
					{
						commonController.messageDialog('초대 완료', '해당 회원이 초대되었습니다. 안내장을 받은 회원이 참여 버튼을 눌러야 초대가 완료됩니다.',
						function()
						{
							//삭제 후 목록 다시 불러오기
							_refreshGridData();
					...
					function()
					{
						commonController.messageDialog('초대 실패', '서버와의 연결이 원활하지 않습니다.');
					});
				});
			});
		...
		function _refreshGridData()
		{
			...
			function(json) // 성공
			{
				...
				{
					commonController.messageDialog('알림', '더 이상 멤버가 없습니다.',
					function()
					{
						//페이지 보정
						--pageNum;
	...
	//초기화
	commonController.boardManagement.memberInvitation.init();

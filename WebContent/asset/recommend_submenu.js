(function(global)
{
	...
	commonController.meetingManagement.recommendMeeting =
	(function()
	{
		//전체 모임 조회
		var urlRecommendMeeting = commonController.defaultPath + "/meeting/recommendMeeting";
		...
		function _getList()
		{
			...
			commonController.ajax.accessData(urlRecommendMeeting, meeting,
			function(json) // 성공
			{
				console.log(json.recommendMeeting);

				if( json.recommendMeeting )
				{

					//초기화
					$('#content').html('<div class="moimMainTitleContainer"><img src="asset/moim_star.png"><span class="moimMainTitle"> 추천하는 모임입니다!</span></div>');

					//모임 리스트
					for(var i in json.recommendMeeting)
					{
						$('#content').append('<div class="moimContainer"><div class="page-curl shadow-bottom"><img src="' + commonController.defaultPath + '/uploaded_files/' + json.recommendMeeting[i]['meePhoto'] + '"></div><div class="moimDescription"><h1>' + json.recommendMeeting[i]['meeName'] + '</h1>' + json.recommendMeeting[i]['meeExplain'] + '<input type="button" name="' + json.recommendMeeting[i]['meeId'] + '" value="자세히 보기"></div></div>');
					}
	...
	//초기화
	commonController.meetingManagement.recommendMeeting.init();
